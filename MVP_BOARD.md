_Last updated: 2026-09-01 · Project Coordinator · Research reaction-network **wizard** + Analytics **bring-your-own-data module** built + **LIVE-integrated** (flag-gated iframes) — delivers A2 / A3 / A7. `#analytics` is now the workspace (supersedes the 08-31 measurements browser). Awaiting deploy-dev; B1 (deploy) + B2 (quarantine write-back) stay the critical path._
...
|---|---|---|---|---|
| 1 | UX / UI | UI/UX Designer | 🟡 | Lock ONE design system; J3 knowledge-graph view; J4 planning flow; honest "preview" states. **2026-08-16 demo-polish:** Research sub-topic chips now dynamic (live `GET /api/corpus`) with honest 0-count grey-out; dev/staging ribbon removed from the mockup. **2026-09-01:** Research reaction-network **wizard** + Analytics bring-your-own-data **module** built and **live-integrated as flag-gated iframes** (`#research` swap + in-app `#analytics` scene); Analytics is now a 4th nav module (NEW badge moved off Simulate). |
...
| A2 | Scenario & user screen-flow creation | UI/UX | 🟢 | **Delivered (2026-08-31 → 09-01).** 6 cross-nav screen-flow wireframes + a journey/IA spec (`UI-UX Designer/`), then the **Research reaction-network journey built as a working 3-step wizard** (`app/dev/research_reaction_network.html`) and **live-integrated** into the `#research` scene (flag-gated iframe; flag OFF = the old phase-picker). |
| A3 | Meat profile comparison vs user data, by cuts **[CP]** | GC-MS preview | 🟡 | **Built as an honest preview + live-integrated (2026-08-31 → 09-01).** The bring-your-own **Analytics workspace** (`app/dev/analytics_workspace.html` — guided **2-step** GC-MS/NMR upload → **Compare Samples** → analytical + sensory results vs a default reference) is now the in-app **`#analytics` module** (3rd nav chip, flag `ff-analytics_module`, iframed). Still illustrative (no real parsing). **Open (to make it real):** users upload real data vs bundled sample, and the reference-data source — decisions 1 & 2 below. |
